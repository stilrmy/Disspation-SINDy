...
def main(param=None,device='cuda:0',opt_mode='PGD',num_sample=100,noiselevel=0,Epoch=200,Epoch0=100,lr=4e-6,lr_step=1e-6,lam0=0.8,lam=0.1,batch_size=128,threshold_d=1e-6,tol=1e-5,display=True):
...
# The gravitational acceleration (m.s-2).
    ...
    def PGD_loop(Tau, coef, prevcoef,d_coef, RHS, Dissip, xdot, bs, lr, lam, momentum=True, D_CAL=False,device='cuda:0'):
        ...
        for i in range(tl//bs):
            ...
            that = (1 + math.sqrt(1 + 4 * t ** 2)) / 2
            vhat = v + ((t - 1) / that) * (v - prev)
            vhat = vhat.clone().detach().requires_grad_(True)
            if D_CAL:
                dhat = d + ((t - 1) / that) * (d - pre_d)
                dhat = dhat.clone().detach().requires_grad_(True)
                pre_d = d
            t = that
            prev = v
    ...
    np.save('/mnt/ssd1/stilrmy/finished_work/SINDy_with_Rdf/loss_log_FISTA_real.npy',loss_log)
